...
# Build forward targets from comma or space separated list in dns_servers
DNS_SERVERS="${dns_servers}"
DNS_SERVERS_CLEAN=$(printf "%s" "$$DNS_SERVERS" | tr '\n' ' ' | tr -s ' ')

FWD_LINE="  forward ."
TLS_PRESENT=0
for S in $$DNS_SERVERS_CLEAN; do
  case "$$S" in
    '' ) continue ;;
  esac
  FWD_LINE="$$FWD_LINE $$S"
...

{
  ...
  if [ -n "${route53_zone_id}" ] && [ -n "${domain_name}" ]; then
    echo "${domain_name} {"
    echo "  route53 ${route53_zone_id}:${domain_name}. {"
    echo "    upstream"
    echo "    fallthrough"
    echo "  }"
    echo "  cache 30"
  ...
  echo "    fallthrough"
  echo "  }"
  if [ "$$FWD_LINE" != "  forward ." ]; then
    if [ $$TLS_PRESENT -eq 1 ] && [ -n "${dns_tls_servername}" ]; then
      echo "$$FWD_LINE {"
